...
// Đảm bảo đường dẫn này đúng với vị trí file logo của bạn
...
const ROOM_DATA = [
  {
    ...
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAirbnfp84pQx1xOoRJhvc9bIqT4iUg0WkxuSbYLPRGCPSzWqNuYqYMc2dnj6ZVZ7faEtsswkHWwOHgIOiSZw-Bkw_FyM4XfyYbUmznWcAXGIGFGb67meUawvtVky86UALMgHjvsoxrti3zfL3AM7pyOCjf55zd3ciCdyyNZw5BH-q-rhlmNbBvGEbryHOQR8oLoEzlB_BZoHO8SrP-NIEQeHLw2N9ducex6Xz4VIvF4QlyTdHL-LpnJ24DRwKBsbR7q5E3RR2wGYli",
    badge: "MỚI",
    rating: 4.92,
    reviews: 128,
    ...
    <div className="space-y-3 max-h-[160px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
      {items.map((item, index) => (
        ...
        >
          <input
            type="checkbox"
            className="w-4 h-4 rounded border-slate-300 text-[#351a5b] focus:ring-[#351a5b] cursor-pointer"
          />
...
// COMPONENT: Thẻ Phòng (Room Card)
const RoomCard = ({ room, viewMode, navigate }) => {
  const isList = viewMode === "list";

  return (
    <div
      className={`bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300 ${isList ? "flex flex-col sm:flex-row" : "flex flex-col h-full"}`}
    >
      <div
      ...
      >
        ...
        />
        {room.badge && (
          <div className="absolute top-2 left-2 bg-[#351a5b] text-white text-[10px] font-bold px-2 py-1 rounded">
            {room.badge}
          </div>
        )}
      </div>
      ...
      <div
        className={`flex-1 flex flex-col justify-between cursor-pointer ${isList ? "p-5" : "p-4"}`}
        onClick={() => navigate(`/room/${room.id}`)}
      >
        <div>
          <div className="flex justify-between items-start">
            <h2 className="text-lg font-bold text-slate-900 line-clamp-2 pr-2">
              {room.title}
            </h2>
      ...
      <div
        className={`bg-slate-50 border-slate-200 flex flex-col justify-end items-end text-right ${isList ? "w-full sm:w-[220px] shrink-0 border-t sm:border-t-0 sm:border-l p-5" : "w-full border-t p-4 mt-auto"}`}
      >
        <p className="text-xl font-extrabold text-[#351a5b]">
          {room.price} VND
        </p>
...
const MapViewPlaceholder = ({ onClose }) => (
  <div className="w-full relative h-[calc(100vh-140px)] bg-slate-200 overflow-hidden">
    ...
    {/* ============================================================== */}
    {/* VỊ TRÍ SAU NÀY BẠN SẼ THÊM BẢN ĐỒ THẬT VÀO                       */}
    {/* ============================================================== */}
    {/* Khi làm bản đồ thật, HÃY XÓA toàn bộ thẻ <div> bên dưới          */}
    {/* và thay thế bằng Component <GoogleMap> hoặc <MapContainer>      */}
    ...
    >
      {/* Các Pin giả lập (Xóa luôn khi có bản đồ thật) */}
...
const Search = () => {
  ...
  // STATE MỚI: Quản lý việc hiển thị bản đồ
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-[#f7f6f8] text-slate-900 font-sans">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-[#351a5b]/10 bg-white/95 backdrop-blur-md flex justify-center">
        <div className="w-full max-w-[1200px] px-4 lg:px-0 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative flex items-center gap-2 border border-slate-300 rounded-full p-2 pl-3 shadow-sm hover:shadow-md transition-all bg-white cursor-pointer">
              <button
      ...
      {/* Logic: Nếu showMap là true -> Hiện bản đồ. Nếu false -> Hiện danh sách như cũ */}
      {showMap ? (
        ...
        <main className="w-full flex justify-center py-8">
          <div className="w-full max-w-[1200px] px-4 lg:px-0 flex flex-col md:flex-row gap-8">
            ...
            <aside className="w-full md:w-[280px] shrink-0 space-y-6">
              {/* NÚT BẬT BẢN ĐỒ */}
              <div
                className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm relative h-32 flex items-center justify-center cursor-pointer group"
                onClick={() => setShowMap(true)}
              >
                <img
                  src="https://lh3.googleusercontent.com/aida-public/AB6AXuAz25RI7mYhyjyoKyUIzusxonlPhTp7q0lg5JTAvJrm5Vhn0qOQ5SStFxfFmREjqkjw61cP-drWGQY-T8niksZyVJberihONFmyzugHmEj8kDvH-XfoAGioNWGpM48nHdE9rD02OksC6aObfUsIMwRk9bgG8a1pYM4L8Ot2Tj6_RhU_2H-uYwvzJO8m_Q7yuGrwxjInHrzjOKLVcny-cy3gQWROUS7PReMlsy4DmC-rhFyXl5pKHaflhYU6sg_GFV-a7ro4u10oigib"
                  alt="Map"
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-[#351a5b] text-sm cursor-pointer hover:underline">
                    Đặt lại
                  </span>
                </div>
                <p className="text-xs text-slate-500 mb-4">Mỗi giờ</p>
                <input
                  ...
                  min="0"
                  max="100"
                  className="w-full accent-[#351a5b]"
                />
            ...
            <section className="flex-1 space-y-4 overflow-hidden">
              <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end mb-6 gap-4">
                <div>
                  <h1 className="text-xl font-bold text-slate-900">
                    Sleepbox tại Hồ Chí Minh
                  </h1>
                  <p className="text-sm text-slate-500">
                    Hơn 300 chỗ nghỉ được tìm thấy
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[#351a5b]">
                      Xem:
                    </span>
              ...
              {/* KHU VỰC RENDER DANH SÁCH (Không còn banner khuyến mãi) */}
              <div
                className={
                  viewMode === "list"
